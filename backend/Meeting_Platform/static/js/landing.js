...
createButton.addEventListener('click', (e) => {
    ...
    console.log("i am before location.href")
...
document.getElementById("createroom").onclick = function () {
    const roomCode = Math.random().toString(36).substring(2, 10); // Generates a random room code
    window.location.href = `/room?roomid=${roomCode}&username=${username}`; // Replace YourUsername with actual username input
};

document.getElementById("joinroom").onclick = function () {
    const roomCode = document.getElementById("roomcode").value;
    const username = "YourUsername"; // Replace with actual username input
    window.location.href = `/room?roomid=${roomCode}&username=${username}`;
};
